...
const Companies = () => {
  ...
  const handleEdit = (company) => {
    // تحديث حالة editCompany لتضمن عرض قيمة المواد في حالة التحديث
    setEditCompany({
      ...company,
      material: company.material._id // أو استخدم قيمة أخرى حسب الحاجة
    });
  };


  // تحديث handleUpdate للتحقق من تحديث الصورة
  const handleUpdate = async (companyId) => {
    try {
      ...
      // تحقق من وجود تغيير في الصورة قبل إضافتها إلى formData
      ...
      fetchCompanies();
      setEditCompany(null); // تحديث حالة editCompany بعد الانتهاء من التحديث
    } catch (error) {
      toast.error(error.response?.data?.message || "Error updating Company");
    }
  };

  // تحديث handleInputChange للتعامل مع تغيير الصورة
  const handleInputChange = (e) => {
    ...
    if (name === "image" && files && files[0]) {
      ...
      setEditCompany((prevCompany) => ({
        ...prevCompany,
        [name]: value,
        // إزالة التحديث الآخر إذا لم يتم تغيير الصورة
