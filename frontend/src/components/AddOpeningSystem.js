...
const AddOpeningSystem = () => {
    ...
    const [image, setImage] = useState(null); // يجب أن يكون null لتخزين الملفات
    const [companies, setCompanies] = useState([]); // استخدم التسمية الجمع لأنك تخزن مجموعة من الشركات
    const [companyId, setCompanyId] = useState(""); // استخدم camelCase للتسمية
    ...
    const handleAddOption = () => {
        if (newOption.trim() === '') return; // تجاهل القيم الفارغة
        setType([...type, { option: newOption, checked: true }]); // إضافة الخيار الجديد إلى القائمة
        setNewOption(''); // إعادة تعيين القيمة الجديدة إلى الفارغة
    };
