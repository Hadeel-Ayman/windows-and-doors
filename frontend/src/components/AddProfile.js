...
const AddProfile = () => {
  ...
  // Fetch companies, materials, and colors on component mount
  useEffect(() => {
    fetchCompanies();
    fetchMaterials();
    fetchColors();
  ...
  // Fetch systems based on selected company
  useEffect(() => {
    if (company) {
  ...
  // Fetch companies based on selected material
  useEffect(() => {
    if (material) {
      ...
      fetchCompanies(); // Fetch all companies if no material is selected
    }
  }, [material]);
